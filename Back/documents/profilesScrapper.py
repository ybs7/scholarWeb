...
def scrape_google_scholar_author_profiles():
    from serpapi import GoogleSearch
    val1 = int(input("enter 1 for authorName or enter 2 for authorid"))
    if val1 == 1:
        str1 = "mauthors"
    else:
        str1= "author_id"
    #xhHbbOsAAAAJ
    str2= input("Enter Authors Info:")
    if val1 == 1:
        params = {
        ...
        author_profiles_data["search_data"][str1] = results.get("search_parameters").get(str1)

        while True:
            results = search.get_dict()

            for profile in results.get("profiles", []):
                print(f"Extracting profile: {profile.get('name')}")

                author_profiles_data["profiles"].append({
            ...
            if "next" in results.get("serpapi_paggination", []):
                search.params_dict.update(
                    dict(parse_qsl(urlsplit(results.get("serpapi_pagination").get("next")).query)))
            else:
                break

        print(json.dumps(author_profiles_data, indent=2, ensure_ascii=False))
        print(
            f"Done. Extracted {len(author_profiles_data['profiles']) - 1} profiles.")  # counts extra empty line, that's why -1.

        # masatoshi_nei_author_articles.csv
        pd.DataFrame(data=author_profiles_data["profiles"]).to_csv(
            f"{author_profiles_data['search_data']['mauthors'].lower().replace(' ', '_')}_author_profiles.csv",
            encoding="utf-8"
        )

        return author_profiles_data
    else:
        params = {
            "api_key": "3123940f5057559598689527ddaa958fec30e00c5b98241487c3c6b55e4ccc06",
        ...
        author_results_data["author_data"]["public_access_available"] = results.get("public_access", {}).get(
            "available")
        author_results_data["author_data"]["public_access_not_available"] = results.get("public_access", {}).get(
            "not_available")
...
scrape_google_scholar_author_profiles()
